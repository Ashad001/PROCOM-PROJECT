	...
	while (window.isOpen())
	{
		...
		if (Is_Game_Pause == 0)
		{
			...
			if (respawn + killCount / 20.f >= 90)
			{
				...
				else if (Power[powerCounter] == '4' && killCount >= 30 * player.GetHp())
				{
					enemy.setColor(sf::Color::Cyan);
				}
				else if (Power[powerCounter] == '5' && killCount >= 130 * bullet_strength)
				{
		...
		if (Is_Game_Start == 1)
		{
			...
			if (player.GetHp() <= 0)
			{
				...
				for (int i = 0; i < countOfScores; i++)
				{
					if (killCount > HighScoresOfPlayers[i])
					{
						int* temp = new int;
						*temp = HighScoresOfPlayers[i];
						HighScoresOfPlayers[i] = killCount;
						killCount = *temp;
						delete temp;
					}
					else if (killCount == HighScoresOfPlayers[i])
					{
						break;
					}
				}
